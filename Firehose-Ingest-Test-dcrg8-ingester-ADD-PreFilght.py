...
FIREHOSE_STREAM_NAME = "PUT-S3-DCRG8-Data"
...
def preflight_device(device):
    ...
    try:
        if not client.connect():
            print(f"[PREFLIGHT] {device['name']} UNREACHABLE - connect failed")
            return False

        # Read one known-good register: PF measured
        val = safe_read(
            lambda: read_input_32(client, 0, signed=True)
        )

        if val is None:
            print(f"[PREFLIGHT] {device['name']} UNREACHABLE - no PF response")
            return False

        print(
            f"[PREFLIGHT] {device['name']} REACHABLE "
            f"(pf_measured={val / 1000.0})"
        )
        return True

    except Exception as e:
        print(f"[PREFLIGHT] {device['name']} UNREACHABLE - {e}")
        return False
...
def poll_device(device):
 ...
 #       }
    ...
    # Updated 2026/07/25:
    # Lovato documented registers after lab validation
    ...
    #        "voltage_l3": (68, True, 10),
        ...
        # ---------------- FREQUENCY ----------------
        # Disabled: register 38 appears to be incorrect for DCRG8.
        # Re-enable only after the authoritative register is confirmed.
...
def ingest_to_firehose(device, data):
    ...
    print(f"[{device['name']}] FIREHOSE OK")
...
#            ingest_to_firehose(device, data)
...
def lambda_handler(event, context):
    ...
    print("[INFO] Starting device preflight")

    for device in devices:

        if preflight_device(device):
    ...
    print(
        f"[INFO] Preflight complete: "
        f"{len(reachable_devices)}/{len(devices)} devices reachable"
    )
