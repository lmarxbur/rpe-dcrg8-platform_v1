...
# ================= LOAD DEVICES =================
DEVICES_FILE = os.getenv("DEVICES_FILE", "/home/ec2-user/modbus_v2.json")
...
def log(level, msg, **kwargs):
    print(json.dumps({"level": level, "msg": msg, "time": datetime.now(timezone.utc).isoformat(), **kwargs}))
...
def write_s3(records):
    ...
    for device, dev_records in by_device.items():
        ...
        key = f"dcrg8-events_v2/{partition}{int(time.time() * 1000)}.parquet"
